-- ============================================================================
-- Report: Active User Info
-- name (used in URL/endpoint): active-user-info    |    id: 11    |    order_no: (none -- sorts last)
-- end_point: active-user-info
-- created_at: 2025-08-04 08:41:30.814215    updated_at: 
...
-- ------------------------------------------------------------------
-- 1. MAIN QUERY  (admin_reports.query, id = 11)
-- ------------------------------------------------------------------
...
-- ------------------------------------------------------------------
-- 2. FILTER PARAMETERS  (admin_reports_parameters WHERE admin_report_id = 11)
-- ------------------------------------------------------------------
-- Filter: Manager Name
...
--   select label, value, sort_order from (select 'All' as label, '#99#' as value, 0 as sort_order union select label, label as value, ROW_NUMBER () OVER (ORDER BY label) as sort_order from (select distinct manager_name as label from vr_active_user_info) a) as t order by sort_order, label
--
-- Filter: Organisation
...
--   input_field    : SelectBox
--   dropdown options are populated by this sub-query:
--   select label, value, sort_order from (select 'All' as label, '#99#' as value, 0 as sort_order union select label, label as value, ROW_NUMBER () OVER (ORDER BY label) as sort_order from (select distinct organisation as label from vr_active_user_info) a) as t order by sort_order, label
--
-- Filter: Vertical
...
--   input_field    : SelectBox
--   dropdown options are populated by this sub-query:
--   select label, value, sort_order from (select 'All' as label, '#99#' as value, 0 as sort_order union select label, label as value, ROW_NUMBER () OVER (ORDER BY label) as sort_order from (select distinct vertical as label from vr_active_user_info) a) as t order by sort_order, label
--
-- Filter: Department
...
--   input_field    : SelectBox
--   dropdown options are populated by this sub-query:
--   select label, value, sort_order from (select 'All' as label, '#99#' as value, 0 as sort_order union select label, label as value, ROW_NUMBER () OVER (ORDER BY label) as sort_order from (select distinct department as label from vr_active_user_info) a) as t order by sort_order, label
--
-- Filter: Branch
...
--   input_field    : SelectBox
--   dropdown options are populated by this sub-query:
--   select label, value, sort_order from (select 'All' as label, '#99#' as value, 0 as sort_order union select label, label as value, ROW_NUMBER () OVER (ORDER BY label) as sort_order from (select distinct branch as label from vr_active_user_info) a) as t order by sort_order, label
--
-- Filter: Designation
...
--   input_field    : SelectBox
--   dropdown options are populated by this sub-query:
--   select label, value, sort_order from (select 'All' as label, '#99#' as value, 0 as sort_order union select label, label as value, ROW_NUMBER () OVER (ORDER BY label) as sort_order from (select distinct designation as label from vr_active_user_info) a) as t order by sort_order, label
--

-- ------------------------------------------------------------------
-- 3. RESULT COLUMN MAPPINGS  (admin_reports_results_mappings WHERE admin_report_id = 11)
-- ------------------------------------------------------------------
...
-- 5. KNOWN ISSUES
-- ------------------------------------------------------------------
-- 1. See report 12 (Inactive User Info) -- it currently runs this exact same query by mistake.

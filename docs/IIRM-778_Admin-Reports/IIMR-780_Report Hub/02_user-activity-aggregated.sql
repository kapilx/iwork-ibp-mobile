-- ============================================================================
-- Report: User Activity Summary
-- name (used in URL/endpoint): user-activity-aggregated    |    id: 2    |    order_no: (none -- sorts last)
-- end_point: user-activity-aggregated
...
-- ------------------------------------------------------------------
-- 1. MAIN QUERY  (admin_reports.query, id = 2)
-- ------------------------------------------------------------------
-- To update, edit the query text below and run:
--   UPDATE admin_reports SET query = $Q$select created_by, entity_type, operation, count(1) as action_count from vr_user_activity where created_at::date BETWEEN ###START_DATE###::date and ###END_DATE###::date group by created_by, entity_type, operation order by created_by, entity_type, operation$Q$
--   WHERE id = 2;

select created_by, entity_type, operation, count(1) as action_count from vr_user_activity where created_at::date BETWEEN ###START_DATE###::date and ###END_DATE###::date group by created_by, entity_type, operation order by created_by, entity_type, operation

-- ------------------------------------------------------------------
-- 2. FILTER PARAMETERS  (admin_reports_parameters WHERE admin_report_id = 2)
-- ------------------------------------------------------------------
-- Filter: Start Date
...
-- ------------------------------------------------------------------
-- 3. RESULT COLUMN MAPPINGS  (admin_reports_results_mappings WHERE admin_report_id = 2)
-- ------------------------------------------------------------------
...
--   action_count                 -> # of Actions                   (variable: count, type: number, align: default)
--   created_by                   -> Performed By                   (variable: performedBy, type: string, align: default)

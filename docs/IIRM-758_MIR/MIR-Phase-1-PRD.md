...
| **Formula** | Policy type, grouped (GAP: as-built groups by policy name; no "critical" filter) | Count of policies in the name group | Sum of inception premium (is-inception endorsement gross premium; else `policy.premium_at_inception`) | CRM-entered | CRM-entered |
...
- <span style="color:#d00">Build note</span>: no "critical" filter is applied — it lists every active policy, and groups by policy **name** not policy **type**. Both must change to match the section's intent.
...
| **Formula** | Policy type (lookup) + insurer policy number; policies expiring within 3 months of the period (GAP: as-built shows policy name, not type) | Premium YTD (GAP: as-built returns inception premium) | `policy.policy_to` | Opportunity activity name (Opty module) — <span style="color:#d00">not implemented (code hardcodes "Active")</span> |
...
- **Renewal Status** = the current **Opportunity activity name** for that policy's renewal (Opty module). <span style="color:#d00">Not implemented — the code returns a constant "Active".</span>
...
- <span style="color:#d00">Build note</span>: as-built, **Start** = not-settled claims from the policy start to the previous month-end and **Received** = not-settled claims from the policy start to the report month-end — two overlapping cumulative windows, so **End = Start + Received − Paid double-counts** the opening base. "Received" and "Paid" should be restricted to *within the report month*.
...
| **Formula** | Policy Type - Policy Number | Inception premium | Duplicates Incep. Premium (GAP: no separate YTD calc) | Sum of claim amount to prev month-end | <span style="color:#d00">Not implemented — blank (intended Claims ÷ Premium)</span> | CRM-entered |
...
- **Premium YTD** = premium year-to-date (intended). <span style="color:#d00">As-built duplicates the inception premium.</span>
- **Claims as on Date** = Σ claim amount for claims dated from the policy start to the **previous month-end**.
- **Loss Ratio** = **Claims as on Date ÷ Premium YTD × 100** (a percentage). <span style="color:#d00">Not implemented — the code leaves it blank.</span>
...
| **Formula** | Policy Type - Policy Number | Outstanding reimbursement (not-settled) claims: count / sum | count / sum aged >45d | count / sum aged 30–45d | count / sum aged 15–30d | count / sum aged ≤15d (GAP: code emits buckets reversed vs these headers) |
...
- <span style="color:#d00">Build note</span>: the code computes the buckets correctly but emits them in ascending order (≤15 first), so in the shipped output the values appear under the wrong descending headers (>45 first). The column order must be fixed.
...
| **Formula** | Policy Type - Policy Number + claim number | GAP: code shows the TAT bucket label here, not policy name | Claim amount | Claim date | CRM-entered (GAP: code lists ALL outstanding reimbursement claims, not only 30–45 day ones) |
...
- <span style="color:#d00">Build note</span>: as-built lists **all** outstanding reimbursement claims (any age) and shows the **TAT-bucket label** in the column meant for the policy identifier. Both must be corrected — filter to 30–45 days and show the policy there.
...
| **Formula** | Policy Type - Policy Number | `policy.policy_to` | Inception premium | <span style="color:#d00">Not implemented</span> | Duplicates Incep. Premium (GAP) | <span style="color:#d00">Not implemented</span> | <span style="color:#d00">Not implemented</span> | Sum of claim amount to prev month-end | <span style="color:#d00">Not implemented</span> |
...
- <span style="color:#d00">Not implemented</span>: **Sum Insured**, **Premium ADDONS** (premium from endorsement additions), **Premium DELONS** (premium from endorsement deletions), and **Insurer**. **Premium YTD** currently duplicates Incep. Premium.
- Once built, **Premium ADDONS/DELONS** = premium added/removed by endorsements during the period; **Sum Insured** and **Insurer** come from the policy and its lead insurer (see §7).
...
- <span style="color:#d00">Not implemented — full section rework</span>: the shipped code (`s9t1`) returns **one row per transaction** with a per-row running balance. It has **no** account-level rollup (per-account opening/closing), **no** month-scoped credit/debit totals, **no** account status, and **no** linked-policies list. This section must be rebuilt to aggregate by CD account.

> ⚠ Technical (Tech Lead review): source is the existing Caution Deposit records (`caution_deposit` / `caution_deposit_transaction`, `/cd-management`) — read-only, but this is a **new account-level aggregation**; the as-built per-transaction ledger is replaced.
...
| **Example** | Endorsement | 340 / 347 | 332 | 3 | 3 | 2 | 7 | — | 9.80 | 10 |

This section renders the **Service Tracker Score**. All formulae — service list, weightages, TAT bucket ranges, row `Scored` math, and the footer Total Service Score — are defined canonically in the **[Service Score PRD §4](../Service-Score/Service-Score-PRD.md)** and must not be restated or modified here. Known build gaps (4 unimplemented services, mislabeled bucket columns) are tracked there in §4.4.
...
5. **CD Account linkage** — confirm a company can map to multiple CD accounts in a single MIR, and how multiple accounts are presented (per-account block vs combined).

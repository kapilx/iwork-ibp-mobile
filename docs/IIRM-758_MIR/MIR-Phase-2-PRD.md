...
| Lead CRM | iWork internal user (`leadCRM` on company) | Creates, fills, submits (same draft-stage access as Associate CRM) — **and** approves / rejects — sole approver, single-level (see BR-MIR-007) |
...
**BR-MIR-004 — Submit locks editing.** MIR can only be unlocked by a Lead CRM rejection. While in Draft (including a post-rejection draft), either the company's Associate CRM or Lead CRM may edit and submit — draft-stage access is not restricted to whichever of the two created the report.
...
## Appendix — Section Calculation Reference (as implemented)

*This appendix documents the actual formulas, filters, and data sources used by `MirReportService.getDynamicSectionRows()` (`apps/services/policy-service/src/app/policy/mir-report.service.ts`) to compute each section at MIR-creation time. It supersedes the narrative "Source: Auto" descriptions above wherever there's a discrepancy — this is the ground truth for how the numbers are actually derived. Section numbers below match the implementation's `sX`/`sXtY` keys, not the §1–§18 numbering above (the implementation ships a subset: s1, s2, s3, s4, s6, s7, s8, s9, s10).*

# Policy Configurator — Business & Functional Requirements (v2)

> **Version history**
> | Version | Date | Changes |
> |---|---|---|
> | v1.0 | 07 May 2026 | Initial release — six-stage wizard (FR-001 through FR-041) |
> | v1.1 | 07 May 2026 | Added Benefit/Waiver Component (FR-042, FR-043, FR-048), Dependent Count parameter (FR-044 through FR-047, FR-049), and Dependent Attribute Premium via `applyToDependents` (FR-050 through FR-053) |
> | v2.0 | 08 July 2026 | **Phase 2** — Added Dependent Attribute Parameter type (FR-054 through FR-059, BR-032 through BR-037) |
> | v2.1 | 09 July 2026 | **Phase 2.1** — Added Family Floater Per-Life Rater support via DependentCount "All" category (FR-060, BR-038, OQ-013) |
> | v2.2 | 13 July 2026 | **Phase 2.2** — Added DA premium pro-ration requirements (FR-061 through FR-063, BR-040 through BR-042, AC-10 through AC-13) |
> | v2.3 | 14 July 2026 | **Phase 2.2 Bug Fix** — Fixed DA premiums not summing for batch-enrolled employees (root cause: IBP batch endpoint bypasses `validateComponentChoices`). Clarified that per-dependent DA pro-ration (FR-061/FR-063, AC-10 through AC-13) is **deferred** — current implementation pro-rates DA as part of `choice.premium` using employee effective date. Added OQ-015. |
...
- **Stage 4 – Policy Parameters:** Definition of employee attribute parameters (Age, Grade, Gender, Marital Status, Designation, Relationship Group, Custom List, Custom Range, Dependent Count, **Dependent Attribute**) that drive premium variation; Cartesian product generation of all resulting policy option combinations
...
- Capture of list-type dependent custom attributes on the employee-facing enrolment form (Phase 3 — tracked separately)
...
**Description:** Administrator optionally defines the employee attribute parameters that drive premium variation. The system generates the Cartesian product of all configured parameter options to produce the full set of policy option combinations that must be priced in Stage 5. Additionally, administrators may define Dependent Attribute parameters that add per-dependent additional premiums at enrolment time without expanding the policy option Cartesian product.
**Actors:** IIRM Administrator
...
flowchart TD
    ...
    B -->|Yes| D[Admin selects parameter type: Age, Grade, Gender, Marital Status, Designation, Relationship Group, Custom List, Custom Range, Dependent Count, Dependent Attribute]
    D --> E{Parameter type?}
    ...
    E -->|Dependent Attribute type| DA[Admin configures Dependent Attribute parameter]
    DA --> DA2[Admin selects Target Relation Category]
    DA2 --> DA3[Admin selects Attribute Kind: Range or List]
    DA3 --> DA4{Attribute Kind?}
    DA4 -->|Range| DA5[Admin defines named range bands: Name, Min, Max, Company Premium, Employee Premium]
    DA4 -->|List| DA6[Admin defines list options: Value, Company Premium, Employee Premium]
    DA5 --> J
    DA6 --> J
    ...
    I --> J
    J -->|Yes| D
    J -->|No| K[System previews Cartesian product count - excludes Dependent Attribute parameters]
    ...
    class A,D,F,G,H,I,L,R,DA,DA2,DA3,DA5,DA6 user;
    class C,K,Q system;
    class S error;
    class B,E,J,M,N,O,P,DA4 decision;
```
...
- Dependent Attribute parameters are excluded from the Cartesian product count preview

---

### **Use Case UC-005: Stage 5 – Configure Policy Choices**

**Description:** For every policy option generated from the Stage 4 parameter Cartesian product, the administrator configures the premium breakdown (company and employee contributions) for every component and every sum insured value. Availability and default flags are set per sum insured option. Dependent Attribute parameters do not generate additional policy options in Stage 5.
**Actors:** IIRM Administrator
...
24. **FR-024:** System shall support the following parameter types:
    ...
    | Dependent Count      | `dependent-count`   | Count-range bands on enrolled member count in a relation category |
    | **Dependent Attribute** | **`dependent-attribute`** | **Custom range or list attribute evaluated per enrolled dependent of a target relation category; adds an additive per-dependent premium** |
...
28. **FR-028:** System shall generate the **Cartesian product** of all configured parameter options. Each resulting combination shall become a distinct **Policy Option** in Stage 5. **Dependent Attribute parameters are excluded from this Cartesian product.**
...
30. **FR-044:** System shall support a new parameter type **"Dependent Count"** with internal type `dependent-count`. The admin selects a **target relation category** (one of: **All**, Self, Spouse/Partner, Children, Parents, Siblings) and defines one or more **count-range bands**. Each band has:
...
36. **FR-052:** Certain parameter types (Grade, Designation, Marital Status, Custom List, Custom Range) hold attributes meaningful only for employees, not dependents. When a parameter of these types has `applyToDependents: true`:
    - The system shall use the **employee's own attribute value** when resolving the policy option for each dependent.
    - Parameters with inherently per-life attributes (Age, Gender) shall resolve each dependent's value from their enrolment record data.
    - The Stage 4 UI shall display an advisory warning when `applyToDependents: true` is set on a Grade, Designation, Marital Status, Custom List, or Custom Range parameter.

37. **FR-053:** When `applyToDependents: true` is set on a parameter, the component's `premiumPerLife` flag shall be ignored for the purpose of per-life multiplication. The effective component premium is always the algebraic sum of each enrolled life's matched premium.

38. **FR-054:** System shall support a new Stage 4 parameter type **"Dependent Attribute"** with `internalType: dependent-attribute`. The administrator configures:
    - **`targetRelationCategory`** — the relation category whose enrolled members' attributes are evaluated (one of: Self, Spouse/Partner, Children, Parents, Siblings).
    - **`attributeKind`** — the kind of attribute evaluation: `range` (numeric bands, e.g., age ranges) or `list` (categorical values).
    - **`targetAttributeName`** — a label describing the attribute being evaluated (e.g., "Age", "Health Tier"). Used for display and attribute-resolution lookup at enrolment time.
    - Bands (for `range`) or options (for `list`), each carrying individual additional premium amounts.

39. **FR-055:** For **Dependent Attribute parameters with `attributeKind: range`**, the admin shall define one or more **attribute range bands**. Each band has:
    - `displayName` — a label for the band (e.g., "51–60 years").
    - `min` — minimum attribute value (inclusive, numeric string).
    - `max` — maximum attribute value (inclusive, numeric string); `null` represents "and above" (unlimited upper bound).
    - `companyAdditionalPremium` — additional company contribution (in ₹) applied per enrolled dependent matching this band.
    - `employeeAdditionalPremium` — additional employee contribution (in ₹) applied per enrolled dependent matching this band.

    Bands must be non-overlapping and contiguous starting from 0.

40. **FR-056:** For **Dependent Attribute parameters with `attributeKind: list`**, the admin shall define one or more **attribute list options**. Each option has:
    - `value` — a categorical string value (e.g., "Metro", "Non-Metro", "Standard", "High Risk").
    - `companyAdditionalPremium` — additional company contribution (in ₹) applied per enrolled dependent matching this option.
    - `employeeAdditionalPremium` — additional employee contribution (in ₹) applied per enrolled dependent matching this option.

41. **FR-057:** A Dependent Attribute parameter shall **not** participate in the Cartesian product generation. It does not create new policy option dimensions in Stage 5. The number of Stage 5 policy options is unaffected by the presence of Dependent Attribute parameters.

42. **FR-058:** At enrolment time, the system shall evaluate each enrolled dependent of the `targetRelationCategory` against the Dependent Attribute parameter:
    - For `attributeKind: range`: resolve the dependent's attribute value (e.g., age computed from DOB for `targetAttributeName: "Age"`), match against range bands, and apply the matched band's `companyAdditionalPremium` and `employeeAdditionalPremium`.
    - For `attributeKind: list`: look up the dependent's value for `targetAttributeName` from their enrolment record, match against list options, and apply the matched option's premiums.
    - The matched additional premiums for each dependent are summed independently and added on top of the base component premium resolved from Stage 5.
    - If no dependents of the `targetRelationCategory` are enrolled, no additional premium is applied for this parameter.

43. **FR-059:** Multiple Dependent Attribute parameters may be defined in a single policy, targeting the same or different relation categories or different attribute kinds. Each parameter is evaluated independently and its additional premiums are accumulated additively. The `applyToDependents` flag is **not** applicable to Dependent Attribute parameters (they are inherently per-dependent).

44. **FR-060:** System shall support a special value **`"All"`** for the `targetRelationCategory` field of a **Dependent Count** parameter. When `"All"` is selected as the target relation category, the system counts all enrolled lives — the employee (always 1) plus all enrolled dependents of every category — to determine which count band applies. This enables **family-floater SI-slab** configuration, where the applicable Sum Insured row increases with every new enrolled life regardless of relation type.

    When combined with an Age parameter that has `applyToDependents: true`, this allows admins to configure a 2D rater table (SI slab × age band) in Stage 5 from which each enrolled life's premium is independently looked up and then summed — matching the premium rater model used by insurers for family-floater policies.

    **Example configuration for a family floater rater:**
    - Stage 4: DependentCount parameter, `targetRelationCategory: "All"`, bands: 1 life → SI 200K / 2 lives → SI 300K / 3 lives → SI 400K / 4+ lives → SI 500K
    - Stage 4: Age parameter (9 bands: 00-25, 26-35, …, 81-120), `applyToDependents: true`
    - Stage 5: 4 × 9 = 36 combinations — admin populates from the insurer's rater table
    - Enrolment (Employee age 36, Spouse age 24, Child age 3): DependentCount resolves to "3 lives (400K)"; each life's age band is resolved independently; total premium = sum of 3 individual lookups

45. **FR-061 *(Phase 2.2)*:** When the component's `proRationEnabled` flag is `true`, DA additional premiums shall be **pro-rated per enrolled dependent using that dependent's own effective enrollment date** — not the employee's effective date. The pro-rated amount for each dependent is:

    `DA_prorated = DA_band_premium × (dep.applicableDays / totalPolicyDays)`

    where `dep.applicableDays = daysBetweenInclusive(dep.effectiveDate, policyTo)` and `totalPolicyDays = daysBetweenInclusive(policyFrom, policyTo)`. Each dependent's pro-rated DA premium is calculated independently and accumulated into the total.

46. **FR-062 *(Phase 2.2)*:** When `proRationEnabled = false` on a component, DA additional premiums shall **not** be pro-rated. The full band premium applies for each matching dependent regardless of enrollment date. No DA refund is issued on mid-policy dependent deletion for such components.

47. **FR-063 *(Phase 2.2)*:** When a dependent is removed mid-policy and the component's `proRationEnabled = true`, the system shall credit a **DA deletion refund** for that dependent:

    `DA_refund = DA_band_premium × (dep.remainingDays / totalPolicyDays)`

    where `dep.remainingDays = daysBetweenInclusive(dep.deletedAt, policyTo)`. Dependents with an active claim are excluded from the deletion-eligible set, consistent with the existing per-life refund exclusion logic.
...
36. **FR-048:** For **benefit/waiver components** (`isBenefitComponent: true`), Stage 5 validation shall **not** lock contribution fields to `0` solely because the Sum Insured value is `0`. The `isAvailable` flag shall govern editability as normal.

37. **FR-049:** For policy options generated from **Dependent Count** parameter bands, Stage 5 shall display the **effective SI** for admin reference: `Effective SI = base component SI + siEnhancement` from the corresponding count band.
...
- AC4: A Grade parameter with `applyToDependents: true` applies the employee's grade band to all dependents; no per-dependent grade lookup is attempted
- AC5: A Gender parameter with `applyToDependents: true` uses each dependent's own gender value from their enrolment template row
- AC6: When `applyToDependents: true`, the `premiumPerLife: true` multiplier is not applied on top of the summed individual premiums
- AC7: Stage 4 displays an advisory warning (non-blocking) when `applyToDependents: true` is set on a Grade, Designation, Marital Status, Custom List, or Custom Range parameter

### **FR-054/055/056/057/058/059 — Dependent Attribute Parameter** *(Phase 2)*

- AC1: Admin can add a "Dependent Attribute" parameter targeting "Parents" with `attributeKind: range` and two bands (51–60, 61–70); Stage 4 validation passes and Stage 5 shows no additional policy options
- AC2: Enrolling Employee (₹17,000 base premium) + Father (age 65, matches 61–70 band, company premium ₹8,000) + Mother (age 58, matches 51–60 band, company premium ₹5,000) results in total premium = ₹17,000 + ₹8,000 + ₹5,000 = ₹30,000
- AC3: Enrolling Employee + Spouse only (no parents) with a Dependent Attribute parameter targeting Parents results in total premium = ₹17,000 (no additional premium applied)
- AC4: Dependent Attribute range bands that overlap (e.g., 51–60 and 58–70) are rejected with a validation error before Stage 4 can be completed
- AC5: Dependent Attribute range bands with a gap (e.g., 51–60 and 62–70, with 61 uncovered) are rejected with a validation error
- AC6: Two Dependent Attribute parameters — one targeting Parents (age range) and one targeting Children (health tier list) — may coexist; both are evaluated independently at enrolment time and their additional premiums are summed
- AC7: A Dependent Attribute parameter with `attributeKind: list` and options ["Standard" → ₹2,000, "High Risk" → ₹5,000] correctly applies ₹5,000 additional premium when a dependent record carries the value "High Risk" for `targetAttributeName`
- AC8: The Cartesian product preview count in Stage 4 does not change when a Dependent Attribute parameter is added or removed
- AC9: The `applyToDependents` toggle is not shown for Dependent Attribute parameters (it is not applicable)

**DA Pro-ration ACs** *(Phase 2.2 — FR-061 / FR-062 / FR-063)*

- AC-10: Enrolling Father from day 1 of a 365-day policy (365 applicable days) with DA band premium ₹8,000 and `proRationEnabled = true` charges the full ₹8,000 (₹8,000 × 365/365).
- AC-11: Adding Father mid-policy on day 181 (185 remaining days) with DA band premium ₹8,000 and `proRationEnabled = true` charges ₹8,000 × 185/365 = **₹4,054.79**. Employee enrolled from day 1 is unaffected — employee base premium uses employee's own effective date.
- AC-12: Removing Father on day 181 (185 remaining days), `proRationEnabled = true` — DA refund = ₹8,000 × 185/365 = **₹4,054.79**. Father carries an active claim → DA refund = ₹0 (claim exclusion applies).
- AC-13: Same removal as AC-12 but `proRationEnabled = false` — DA refund = **₹0**; full DA premium was already charged at enrollment and no refund is issued.

### **FR-060 / BR-038 / BR-039 — DependentCount "All" Category (Family Floater Rater)** *(Phase 2.1)*

- AC1: The Stage 4 Dependent Count parameter "Target Relation Category" dropdown includes "All" as a selectable option
- AC2: Selecting "All" shows an informational note: "Counts all enrolled lives (employee + all dependents)"
- AC3: With `targetRelationCategory: "All"` and bands (1 life → SI 200K, 2 lives → SI 300K, 3 lives → SI 400K, 4+ lives → SI 500K): an employee enrolling alone matches the 1-life band; enrolling with a spouse matches the 2-lives band; enrolling with a spouse and a child matches the 3-lives band
- AC4: Combining a DependentCount "All" (4 SI slab bands) with an Age parameter (9 age bands, `applyToDependents: true`) generates exactly 36 policy options in Stage 5
- AC5: Enrollment scenario — Employee (age 36), Spouse (age 24), Child (age 3): DependentCount "All" resolves to 3-lives band (SI 400K); age bands resolve independently per life; total premium = rater[400K][36-45] + rater[400K][00-25] + rater[400K][00-25] = 9302.55 + 4442.21 + 4442.21 = ₹18,186.97
- AC6: Existing Dependent Count parameters targeting named categories (e.g., "Parents", "Children") are unaffected by the addition of an "All" category parameter; both may coexist on the same policy
- AC7: Adding Child 2 to the above enrollment shifts the DependentCount "All" band to 4+ lives (SI 500K); each enrolled life's premium is re-resolved at the 500K row
...
classDiagram
    ...
    class DependentAttributeConfig {
        +string targetRelationCategory
        +string attributeKind
        +string targetAttributeName
    }

    class DependentAttributeRangeBand {
        +string id
        +string displayName
        +number min
        +number max
        +number companyAdditionalPremium
        +number employeeAdditionalPremium
    }

    class DependentAttributeListOption {
        +string id
        +string value
        +number companyAdditionalPremium
        +number employeeAdditionalPremium
    ...
    PolicyParameter "1" --> "0..1" DependentAttributeConfig : dependent attribute config
    DependentAttributeConfig "1" --> "0..*" DependentAttributeRangeBand : range bands
    DependentAttributeConfig "1" --> "0..*" DependentAttributeListOption : list options
...
{
  "id": "string",
  "parameterType": "Age | Grade | RelationshipGroup | Gender | MaritalStatus | Designation | CustomList | CustomRange | DependentCount | DependentAttribute",
  "internalType": "list | range | relation | dependent-count | dependent-attribute",
  "label": "string",
  ...
  "dependentCountConfig": {
    "targetRelationCategory": "All | Self | Spouse/Partner | Children | Parents | Siblings",
    "countBands": [
...
> **Note on `DependentAttribute` parameter:** `dependentAttributeConfig` is only populated when `internalType` is `dependent-attribute`. `max: null` on a range band represents an unlimited upper bound ("and above"). The `companyAdditionalPremium` and `employeeAdditionalPremium` on each band/option are the **additive** premiums applied per enrolled dependent matching that band; they are NOT the total component premium. The `applyToDependents` flag is not applicable and should be ignored for this parameter type.
...
- **BR-025:** Benefit/waiver components are restricted to type `optional`. They cannot be the `base` or `parental` component of a policy.
...
- **BR-027:** The `siEnhancement` value on a Dependent Count band is **additive** over the base SI defined in Stage 1. The effective SI applied at enrolment is: `Effective SI = base component SI + siEnhancement`.
...
### **Dependent Attribute Premium Rules (`applyToDependents`)**

- **BR-029:** When `applyToDependents: true` on a parameter, each enrolled dependent's premium contribution for the affected component is resolved independently by matching that dependent's own attribute value against the parameter bands.
- **BR-030:** When `applyToDependents: true` on a parameter, the `premiumPerLife` flag on the component is implicitly superseded for that component's premium calculation.
- **BR-031:** `applyToDependents` is independent of the Dependent Count parameter type.

### **Dependent Attribute Parameter Rules** *(Phase 2)*

- **BR-032:** The Dependent Attribute parameter does not participate in the Cartesian product. It does not generate new policy option dimensions. The number of policy options in Stage 5 is unaffected by the presence of a Dependent Attribute parameter.
- **BR-033:** The additional premiums defined on Dependent Attribute bands are **additive** over the base component premium resolved from Stage 5. The final component premium for an enrolment is: `Base Premium (from Stage 5 choice) + Σ(companyAdditionalPremium for each matching enrolled dependent) + Σ(employeeAdditionalPremium for each matching enrolled dependent)`.
- **BR-034:** Each enrolled dependent of the `targetRelationCategory` is evaluated independently against the Dependent Attribute parameter bands. If the dependent's attribute matches band X, band X's additional premium is added. If multiple dependents match the same or different bands, all their individual additional premiums are summed. No "take maximum" or "take minimum" strategy is applied — all matching dependents contribute.
- **BR-035:** For Dependent Attribute parameters with `attributeKind: range` and `targetAttributeName: "Age"`, the dependent's age is computed from their date of birth at the time of enrolment (same computation as for the existing Age parameter). For other attribute names, the value is looked up from the dependent's enrolment record.
- **BR-036:** If no dependents of the `targetRelationCategory` are enrolled in the policy, the Dependent Attribute parameter contributes zero additional premium. The base policy option premium applies unchanged.
- **BR-037:** The `applyToDependents` flag does not apply to Dependent Attribute parameters. Dependent Attribute parameters are inherently per-dependent and always evaluate each enrolled dependent of the target category independently.

### **Family Floater Rater Rules** *(Phase 2.1)*

- **BR-038:** When a Dependent Count parameter is configured with `targetRelationCategory: "All"`, the enrolled count used to match count bands is `1 (employee, always present) + total enrolled dependents across all categories`. Category-specific Dependent Count parameters targeting named categories are evaluated independently and are unaffected by this rule.

- **BR-039:** A Dependent Count parameter with `targetRelationCategory: "All"` participates in the Cartesian product in the same way as any other Dependent Count parameter. Each count band becomes a distinct policy option dimension. When combined with an Age parameter (`applyToDependents: true`), the resulting Stage 5 grid represents the 2D family-floater rater (SI slab × age band), and the per-life age resolution + summing is handled by the existing FR-051/FR-053 mechanism.

### **DA Pro-ration Rules** *(Phase 2.2)*

- **BR-040:** DA premium pro-ration is applied at the **individual dependent level**. Each enrolled dependent's matched DA band premium is independently scaled by `dep.applicableDays / totalPolicyDays`. Multiple dependents in the same relation category with different effective dates each produce a different pro-rated DA amount — the amounts are never averaged or aggregated before pro-ration.

- **BR-041:** DA deletion refunds use `dep.deletedAt` as the effective removal date. Dependents with an active claim are excluded from the deletion-eligible set, consistent with the existing `resolveDeletionEligibleLivesCount` logic used for per-life base premium refunds. A DA refund of zero is issued for any claim-excluded dependent.

- **BR-042:** When `proRationEnabled = false` on a component: DA additional premiums are charged in full regardless of each dependent's enrollment date, and a DA refund of zero is credited on mid-policy dependent deletion. This mirrors the existing behaviour for the base component premium when `proRationEnabled = false`.
...
- **ASM-003:** The list of relationship sub-categories available in Stage 2 is a fixed master set and is not configurable per organisation
- **ASM-004:** Employee attribute values that are referenced by parameters in Stage 4 are accurately maintained in the employee data store at the time of enrolment
...
- **ASM-006 (Phase 2):** For Dependent Attribute parameters with `attributeKind: range` and `targetAttributeName: "Age"`, dependent age is always derived from the dependent's date of birth stored in the enrolment record. For `attributeKind: list`, the dependent's value for the custom attribute must be present in the dependent's enrolment record at the time of premium resolution.

### **Dependencies**

- **DEP-001:** Employee Data Store — Stage 4 parameters must resolve against employee profile attributes at enrolment time
- **DEP-002:** Enrolment Service — consumes the saved policy configuration to drive the employee-facing enrolment portal
- **DEP-003:** Premium Calculation Engine — applies the pro-ration formula, per-life/per-family multiplier, MULTIPLE model per-mille rate, and **Dependent Attribute additive premium** using the configuration saved by the Policy Configurator
- **DEP-004:** Policy record — the Policy Configurator operates on an existing policy record
- **DEP-005:** IIRM Authentication & Authorisation — the wizard is accessible only to authenticated administrators
- **DEP-006 (Phase 2):** Dependent Enrolment Record — for Dependent Attribute list-type parameters, the dependent's enrolment record must carry a value for `targetAttributeName`. Population of this field requires a Phase 3 enhancement to the employee-facing enrolment form.
...
| **Policy Relationships** | The set of family relationship categories and sub-categories eligible for enrolment under the policy. |
| **Policy Template** | The mapping that associates optional add-on components with their parent components and assigns eligible relationship types to each component. |
| **Policy Parameters** | Employee attributes configured in Stage 4 to drive premium variation across different employee segments. |
| **Relationship Group** | A named group of family relation categories with associated max counts, used as a `relation`-type parameter. |
| **Cartesian Product** | The set of all possible combinations of parameter option values. Each combination becomes a distinct Policy Option in Stage 5. |
...
| **Universal Option** | The single Policy Option generated when no Cartesian-product parameters are configured in Stage 4. |
| **Policy Choice** | The premium configuration for a specific sum insured option within a specific component within a specific Policy Option. |
| **Per-mille (‰) Rate** | The contribution rate used for `MULTIPLE` model components, expressed as the premium amount per ₹1,000 of computed sum insured. |
| **Club Sum Insured** | A flag on the policy template that, when enabled, aggregates the add-on component's sum insured with the parent component's sum insured. |
| **familyMaxPolicyLevel** | The overall maximum number of lives that can be enrolled under a single policy record. |
| **familyMaxCount** | The maximum total number of lives allowed within a specific Relationship Group. |
| **Benefit Component** | An optional add-on component (`isBenefitComponent: true`) with a Sum Insured of `0`, representing a benefit or waiver rather than a coverage amount. |
| **Dependent Count Parameter** | A Stage 4 parameter type (`internalType: dependent-count`) that defines count-range bands over enrolled members of a specific relation category (or **all enrolled lives** when `targetRelationCategory: "All"`). Each band carries an SI enhancement and generates a distinct policy option in Stage 5. |
| **Family Floater Per-Life Rater** | A premium configuration model where total family SI increases per enrolled life, and each life's individual premium is looked up from a 2D rater table (SI slab × age band) then summed. Configured via a DependentCount "All" parameter + Age parameter with `applyToDependents: true`. |
| **SI Enhancement** | An additive sum insured increment defined on a Dependent Count band. |
| **Count Band** | A named range within a Dependent Count parameter, defined by `minCount`, `maxCount`, and `siEnhancement`. |
| **Effective Enrolment Date** | The date from which coverage is considered active for an employee. Used in pro-ration calculations. |
...
| **Final Review** | A read-only summary screen in Stage 6 displaying the complete policy configuration before the administrator saves. |
| **`applyToDependents`** | A boolean flag on a Stage 4 parameter (default `false`). When `true`, each enrolled life is independently evaluated against the parameter bands and their individually matched Stage 5 premium is summed. |
| **Dependent Attribute Parameter** | A Phase 2 Stage 4 parameter type (`internalType: dependent-attribute`). Targets a specific relation category and evaluates a custom range or list attribute on each enrolled dependent of that category. Adds an additive per-dependent additional premium on top of the base policy option premium. Does NOT generate Cartesian product dimensions. |
| **Attribute Kind** | The evaluation mode of a Dependent Attribute parameter: `range` (numeric bands, e.g., age ranges) or `list` (categorical values). |
| **Target Attribute Name** | The label identifying which attribute of the dependent is evaluated in a Dependent Attribute parameter (e.g., "Age", "Health Tier"). For `attributeKind: range` with `targetAttributeName: "Age"`, the value is computed from the dependent's DOB. |
| **Additional Dependent Premium** | The per-dependent additive premium defined on each Dependent Attribute band or option. Expressed as company and employee contributions separately. Summed across all enrolled dependents matching the target relation category. |
...
| OQ-011 (Phase 2) | For Dependent Attribute parameters with `attributeKind: list`, how does the employee-facing enrolment form capture the dependent's value for the custom attribute? Should a Phase 3 ticket be raised for the ibp-service enrollment form to support this? | Product / Engineering | **Open** |
| OQ-012 (Phase 2) | Should the Final Review screen in Stage 6 display a summary of configured Dependent Attribute parameters and their band premiums? | Product | **Open** |
| OQ-013 (Phase 2.1) | When a DependentCount "All" parameter is active and a dependent is added mid-policy (endorsement), should the SI slab shift be pro-rated? The delta between the old slab premium and the new slab premium for existing enrolled lives would need pro-ration treatment. | Product / Actuarial | **Open** |
| OQ-014 (Phase 2.1) | Should multiple DependentCount "All" parameters be allowed on the same policy, or should the UI restrict to at most one? | Product | **Open** |
| OQ-015 (Phase 2.2) | ~~Per-dependent DA pro-ration not yet implemented.~~ **RESOLVED (14 July 2026):** Per-dependent DA pro-ration implemented inside `calculateProratedPremiumsForEnrollment` (T-13). Each dependent's DA premium is now pro-rated by their own `effectiveDate` (addition) and `deletedAt` (deletion refund) for endorsement PDC calculations. Non-PPL (per-family) policies are fully supported; PPL (per-life) policies use an approximation (existing per-dep loop already uses dep effectiveDate). AC-10 through AC-13 are met for non-PPL policies. | Engineering / Product | **Closed** |

---

*Document version: v2.3 — Updated: 14 July 2026*

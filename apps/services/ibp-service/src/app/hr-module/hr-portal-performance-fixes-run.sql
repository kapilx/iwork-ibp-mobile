-- =============================================================================
-- HR Portal Performance Fixes — DBeaver / pgAdmin version
-- THIS FILE IS FOR COPY-PASTE EXECUTION IN DBEAVER OR PGADMIN ONLY.
--
-- DIFFERENCE FROM hr-portal-performance-fixes.sql:
--   All CREATE INDEX CONCURRENTLY replaced with CREATE INDEX
--   (CONCURRENTLY cannot run inside a transaction block — DBeaver wraps in one).
--
...
-- =============================================================================
CREATE INDEX IF NOT EXISTS idx_group_company_map_company_id
  ON group_company_map (company_id);

CREATE INDEX IF NOT EXISTS idx_group_company_map_group_company_id
  ON group_company_map (group_company_id);

CREATE INDEX IF NOT EXISTS idx_policy_company_id
  ON policy (company_id);

CREATE INDEX IF NOT EXISTS idx_policy_type_lid
  ON policy (policy_type_lid);

CREATE INDEX IF NOT EXISTS idx_policy_enrollment_emp_company_id
  ON policy_enrollment_employee (company_id)
  WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_cpcl_company_id
  ON company_policy_configuration_location (company_id);

CREATE INDEX IF NOT EXISTS idx_cpcl_address_id
...
-- Problem : uses pee_f.policy_config_location_id = ANY(address_ids) but policy_config_location_id
--           stores cpcl.id not address.id — Fix 3 (loc_id_set) does NOT apply here.
UPDATE admin_reports
SET
  query = REPLACE(
    query,
...
CREATE INDEX IF NOT EXISTS idx_peepm_policy_id
  ON policy_enrollment_employee_policy_map (policy_id)
  WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_ped_policy_id
  ON policy_enrollment_dependent (policy_id)
  WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_pee_policy_id
  ON policy_employee_enrollment (policy_id)
  WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_ual_user_id_activity
...
CREATE INDEX IF NOT EXISTS idx_endorsement_policy_id
...
CREATE INDEX IF NOT EXISTS idx_policy_insurer_map_policy_id
  ON policy_insurer_map (policy_id);

-- Fix 11e: policy_tpa_map had no policy_id index — full table scan on every call
CREATE INDEX IF NOT EXISTS idx_policy_tpa_map_policy_id
...
CREATE INDEX IF NOT EXISTS idx_policy_claim_policy_id ON policy_claim (policy_id) WHERE deleted_at IS NULL;
...
CREATE INDEX IF NOT EXISTS idx_ual_auth_login_user
...
CREATE INDEX IF NOT EXISTS idx_pee_policy_employee_updated
...
CREATE INDEX IF NOT EXISTS idx_policy_enrollment_dep_policy_id
ON policy_enrollment_dependent(policy_id)
WHERE deleted_at IS NULL;
...
-- =============================================================================
CREATE INDEX IF NOT EXISTS idx_pee_enrollment_employee_updated

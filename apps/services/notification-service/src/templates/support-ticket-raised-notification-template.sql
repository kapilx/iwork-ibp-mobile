...
VALUES (
  ...
  'Email sent to a user after a support ticket is raised successfully — works for both authenticated employees and anonymous (pre-login) users.'
)
...
VALUES
  ...
  ('description',      'Short description / escalation text from the form'),
  ('submissionDate',   'Date the ticket was submitted'),
...
WITH event AS (
  SELECT id FROM notification_event_type WHERE name = 'Support_Ticket_Raised_Email'
),
...
WITH resolved AS (
  SELECT
    (SELECT id FROM notification_event_type WHERE name = 'Support_Ticket_Raised_Email') AS event_type_id,
...
template AS (
  SELECT
    'Your Support Ticket {{ticketId}} Has Been Raised - IIRM Insurance Wellness Hub'::text AS subject,
    $template$<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>Support Ticket Confirmation - IIRM</title>
  <style>
    ...
    .badge-open { background-color: #1f9d55; }
      ...
      <table class="email-container" width="600" cellpadding="0" cellspacing="0" border="0">
        ...
        <tr>
          <td class="content">
            ...
            <h2 class="main-title">Support Ticket Raised Successfully</h2>
            <p class="sub-text">Your support request has been received. Our team will review it and get back to you shortly. Please keep your ticket ID handy for any follow-up.</p>
          </td>
        </tr>
        <tr>
          <td class="section">
            ...
            <table class="info-card" width="100%" cellpadding="0" cellspacing="0" border="0">
              <tr>
                <td class="info-card-cell">
                  <p class="line"><span class="badge badge-open">OPEN</span></p>
                  <p class="line"><strong>Ticket ID:</strong> {{ticketId}}</p>
                  <p class="line"><strong>Category:</strong> {{category}}</p>
                  <p class="line"><strong>Submitted On:</strong> {{submissionDate}}</p>
                  {{#if description}}<p class="line"><strong>Description:</strong> {{description}}</p>{{/if}}
                </td>
              </tr>
            </table>
          </td>
        ...
        <tr>
          <td class="section">
            ...
            <table class="info-card" width="100%" cellpadding="0" cellspacing="0" border="0">
              <tr>
                <td class="info-card-cell">
                  <p class="line">Our support team will review your ticket and respond to <strong>{{recipientEmail}}</strong> soon.</p>
                  <p class="line">You can also reach out to your HR team or TPA coordinator for urgent queries.</p>
                </td>
              </tr>
            </table>
          </td>

-- Enrolment confirmation mail — Support Contacts section.
--
-- Event type: Enrollment_Confirmation_Email
-- Swaps the old plain "Support" section for the same card the life event
-- confirmation mail renders, in place, so the section keeps its position in the
-- mail. Safe to re-run: a body that already carries the card is left untouched.

BEGIN;

UPDATE notification_channel_event_template_mapping AS m
SET body = CASE WHEN m.body LIKE '%Primary Contact%' THEN m.body ELSE
      -- 2. Drop the trailing enrolment thank-you line, which the card replaces.
      regexp_replace(
      -- 1. Replace the whole old Support <tr> with the new card. The pattern ends
      --    on the support sentence, which occurs exactly once, so the greedy `.*`
      --    cannot run past the block. (Postgres makes the whole RE greedy when the
      --    first quantifier is greedy, so a bare `.*?` here would not be safe.)
...
{{/if}}
      ...
      '\s*Thank you again for completing your enrolment on time\.\s*<br /><br />', '')
    END,
    updated_by = 1, updated_at = CURRENT_TIMESTAMP
FROM notification_event_type AS e
WHERE m.event_type_id = e.id AND e.name = 'Enrollment_Confirmation_Email';

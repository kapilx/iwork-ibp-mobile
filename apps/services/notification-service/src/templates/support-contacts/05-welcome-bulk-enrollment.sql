...
<table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="border-collapse:separate;background:#f8f8f8;border:1px solid #d9dde3;border-left:5px solid #1a73e8;border-radius:10px;margin:0 0 14px;">
  <tr><td style="padding:18px 22px;font-size:14px;line-height:1.65;color:#3d4b59;font-family:Arial,Helvetica,sans-serif;">
    For any changes in the enrolment, please contact your HR team or TPA coordinator.
  </td></tr>
</table>
<table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="border-collapse:separate;background:#f8f8f8;border:1px solid #d9dde3;border-left:5px solid #1a73e8;border-radius:10px;margin:0 0 14px;">
...
{{/if}}$block$) END,
      ...
      'For any queries related to enrolment, coverage, or claims, please contact your HR team or TPA coordinator.', 'For any changes in the enrolment, please contact your HR team or TPA coordinator.'),
      '<(p|div)[^>]*>[^<]*For any changes in the enrolment, please[^<]*</(?:p|div)>', ''),
    updated_by = 1, updated_at = CURRENT_TIMESTAMP
FROM notification_event_type AS e
WHERE m.event_type_id = e.id AND e.name = 'Bulk_Enrollment_Confirmation_Email';

// Shared password validation logic (no NestJS dependencies)
...
export class PasswordValidationUtil {
  static validatePassword(password: string, rules: PasswordRuleDefinition[]): ValidationError[] {
    ...
    for (const rule of sortedRules) {
      const error = PasswordValidationUtil.validateAgainstRule(password, rule);
      if (error) {
        return [error];
      }
    }
  ...
  static isPasswordValid(password: string, rules: PasswordRuleDefinition[]): boolean {
    const errors = PasswordValidationUtil.validatePassword(password, rules);

...
export const tableSearchConfig = (
  ...
  // Owner + Owner-view fields removed: on the Enhanced page the org-scope
  // widget's Owner accordion is the single owner channel (card click + its
  // Manager / Manager + Team toggle → ownerId/viewBy listing params) — form
  // fields here would fight it.
  {
...
// Companies table (portfolio-style flow): one row per company with the RO
...
export const getCompanyColumns = (localization: any) => [
  ...
  {
    headerName: "Total ROs",
    field: "totalRos",
    headerTooltip: "Total ROs",
    width: 140,
    valueFormatter: ({ value }: any) =>
